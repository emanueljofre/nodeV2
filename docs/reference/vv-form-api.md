...
| `SetFieldValue(fieldName, value)`        | 2      | Sets a field value. For calendar fields, goes through `normalizeCalValue()` → `calChange()` → `getSaveValue()`. Triggers `UnsavedChanges` increment. **Silently fails for non-existent fields** (no error, value is discarded). **Returns a Promise under V2** (`useUpdatedCalendarValueLogic=true`) — `await VV.Form.SetFieldValue(...)` is needed for sequenced operations; fire-and-forget leaves the value object mutating in the background. Under V1 the call is synchronous. See FORM-BUG-8 for the `value=""`/`value=null` never-resolves edge case. **SFV(null) on V2 Config D** produces a raw/api asymmetry even when the Promise resolves — `VV.Form.VV.FormPartition.getValueObjectValue(field)` returns literal `null`, while `VV.Form.GetFieldValue(field)` returns `""`; a silent `RangeError: Invalid time value` fires inside the cal-change RxJS pipeline (`getSaveValue → toISOString`) and is caught by the subscriber — not propagated to the caller. Confirmed 2026-04-22 on vv5dev build `20260418.1`. |
