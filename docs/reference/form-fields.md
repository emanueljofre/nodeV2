...
| FORM-BUG-V2-EPOCH-PRESERVED     | V2 preserves epoch-ms raw as number; GFV returns null                                  | `useUpdatedCalendarValueLogic=true` (V2), Configs A and C confirmed                                 | Medium — under V2, `SetFieldValue(field, <number>)` with an epoch-ms value preserves the number in the raw partition (`getValueObjectValue()` returns the number itself, stringified as `"1773543600000"`), whereas V1 coerced it to an ISO/date string. Side effect: V2 `GetFieldValue()` returns **null** for the same field because the V2 GFV pipeline does not handle numeric raws — it only unwraps string-valued partitions. V1 returned the normalized ISO/date string from GFV. Confirmed 2026-04-22 on vv5dev (build `20260418.1`) with `7-A-epoch.V2` (Config A, date-only) and `7-C-epoch.V2` (Config C, DateTime). **Impact**: any script that does `SetFieldValue(f, Date.now())` and then reads back via `GetFieldValue(f)` gets null on V2 where V1 gave a parseable string. **Workaround**: stringify epochs before SFV (`SetFieldValue(f, new Date(epoch).toISOString())`), or read via `VV.Form.VV.FormPartition.getValueObjectValue(f)` which returns the stored number. Root cause TBD — likely a type guard in the V2 GetFieldValue implementation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
