...
- **Date-only vs DateTime**: V1 stored Config A/B as `"2026-03-15"`; full-V2 stores an ISO datetime with `.000Z` suffix, but the actual instant differs by `ignoreTimezone`:
    - **Config A** (`ignoreTimezone=false`): stores **UTC of local midnight** — varies by browser TZ. BRT (UTC−3) → `"2026-03-15T03:00:00.000Z"`, UTC0 → `"2026-03-15T00:00:00.000Z"`, IST (UTC+5:30) → `"2026-03-14T18:30:00.000Z"` (FORM-BUG-7 day shift). Verified 2026-04-20 on vv5dev/EmanuelJofre across all 3 TZs.
    - **Config B** (`ignoreTimezone=true`): stores **UTC midnight regardless of local TZ** → `"2026-03-15T00:00:00.000Z"` always. This makes B the FORM-BUG-7 mitigation under V2 (same role it played under V1).
...
| FORM-BUG-V2-EPOCH-PRESERVED                              | V2 preserves epoch-ms input through raw + GFV instead of coercing to ISO/date          | `useUpdatedCalendarValueLogic=true` (V2), Configs A and C confirmed                    | Medium — under V2, `SetFieldValue(field, <number>)` with an epoch-ms value preserves the stringified numeric epoch (`"1773543600000"`) in both the raw partition and `GetFieldValue()`, whereas V1 normalized it to an ISO/date string (`"2026-03-15"` for Config A, `"2026-03-15T00:00:00"` for Config C). Confirmed 2026-04-22 on vv5dev (build `20260418.1`) with `7-A-epoch.V2` (Config A, date-only) and `7-C-epoch.V2` (Config C, DateTime). **Impact**: scripts that do `Date.parse(gfv)` after an epoch-ms SFV will silently produce `NaN` on V2 (epoch strings are not valid inputs to `Date.parse`), where V1 returned a parseable date string. **Workaround**: wrap GFV with `new Date(+gfv)` when the input may have been an epoch, or stringify the epoch before SFV (`SetFieldValue(f, new Date(epoch).toISOString())`) to force V2 into the ISO path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| FORM-BUG-V2-URL-PARAM-NORMALIZE                          | V2 URL-param init normalizes US-format dates to UTC ISO, ignoring the config's TZ flag | `useUpdatedCalendarValueLogic=true` (V2), Config A (date-only, `ignoreTimezone=false`) | Low–Medium — under V2, a URL parameter like `?Field7=03/15/2026` on a Config A field stores `"2026-03-15T00:00:00.000Z"` (UTC midnight, TZ-agnostic). V1 stored the US-format string as-is (`"03/15/2026"`). **The interesting detail**: this is NOT the UTC-of-local-midnight that SFV or typed input produces for the same field (which would be `"2026-03-15T03:00:00.000Z"` on BRT because of FORM-BUG-7 persisting under V2). The URL-param init path treats the value as TZ-naive — effectively behaving as if `ignoreTimezone=true` even when the field has `ignoreTimezone=false`. Confirmed 2026-04-22 on vv5dev with `4-A-us-BRT.V2`. **Impact**: the same form field stores different UTC values depending on whether it's populated via URL param vs. SFV/typed input — a divergence from V1's consistent pass-through behavior. Not data-lossy (calendar date is preserved), but unexpected for anyone building URL-param-driven workflows.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ~~FORM-BUG-V2-SAVE-RELOAD-EMPTY~~ (withdrawn 2026-04-22) | —                                                                                      | —                                                                                      | **Not a platform bug.** Originally reported after 4 cat-14/cat-16 V2 tests observed empty values after save+reload. Root cause traced to `testing/helpers/vv-form.js saveFormAndReload()` — the helper called `page.reload()` which re-loaded the current browser URL. Under V2, Angular does not push `DataID` into the browser URL on save (the URL stays at `?formid=...` template mode), so `page.reload()` loaded a fresh empty template instead of the saved record. Fix: helper now navigates explicitly to the saved-record URL returned by `saveFormOnly()`. After the fix, the 4 affected tests observe the real V2-persisted values (e.g., `"2026-03-15T00:00:00.000Z"` for Config C BRT, `"2026-03-14T03:00:00.000Z"` for Config A BRT via FORM-BUG-7-persists-on-V2). cat-4-reload tests were previously passing for the wrong reason — their URL contained `&Field7=...` params so `page.reload()` re-applied them after save. The V2 "URL does not include DataID after save" behavior is itself a platform observation worth documenting for anyone relying on browser URL to reflect post-save state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
