...
Generated: 2026-04-16 | Rules: 49 | Findings: 34 (4 errors, 15 warnings, 15 info)
...
| accessibility-required | Was This a Violation | Page 1 | Required field AccessibilityLabel "Was This a Violation dropdown" should end with "field Required" |
| distance-to-border | Top Form ID | Page 1 | Field is 10px from the right border (minimum: 30px) |
| distance-to-border | btnHelp | Page 1 | Field is 25px from the right border (minimum: 30px) |
| button-min-size | btnHelp | Page 1 | Button is 15x15px — minimum is 24x24px (508 compliance) |
| accessibility-label-match | Was This a Violation | Page 1 | AccessibilityLabel "Was This a Violation dropdown" does not match expected "Was this a violation?" |
...
| label-truncation | DataField1 | Page 1 | Label text "Select from the dropdown the WAC that caused this NTC or use the Other field to record a WAC or RCW that is not listed. For each entry, choose whether it is a violation or non-violation." (~1042px) may be truncated in 862px width — increase width or enable wrapping |
| label-truncation | Label23 | Page 1 | Label text "Was this a violation?<strong style="color:#9E0000;">*</strong>" (~434px) may be truncated in 142px width — increase width or enable wrapping |
...
| field-max-length | Other | Page 1 | TextArea MaxLength is 1000 — recommended minimum is 3000 for notes/text fields |
| font-consistency | Label1 | Page 1 | Label style differs from form pattern: fontSize=18 (expected 10), bold=true (expected false), color=#ffffffff (expected #FF000000) |
...
| data-lookup-in-properties | Was This a Violation | — | Script "Was_This_a_Violation_onChange" on drop-down contains API calls — prefer configuring data lookups in the properties panel |
