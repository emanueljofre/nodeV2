...
**V1/V2 parity confirmation (2026-04-22 · vvdemo / EmanuelJofre, build fingerprint `b18dbfdb`)**: same regression re-run against the `DateTest` form on V1 (`useUpdatedCalendarValueLogic=false`). Audit output is **byte-identical** to vv5dev's at all 79 audited slots — same counts, same per-slot verdicts. Two independent platform builds on different code paths produce the same WS API observations everywhere. This confirms (a) the `useUpdatedCalendarValueLogic` toggle affects only the Forms Angular pipeline, not the REST API surface; (b) the matrix expected values remain correct on current vvdemo; (c) the 15 UNFLAGGED and 4 NOT_IN_MATRIX entries are pipeline/matrix artifacts shared across environments, not platform bugs. Artifacts: [`projects/emanueljofre-vvdemo/testing/date-handling/web-services/v2-baseline-audit.md`](../../../../projects/emanueljofre-vvdemo/testing/date-handling/web-services/v2-baseline-audit.md).
