...
| **Web Services (REST API)**         | In Progress | `web-services/` (148 baselined on WADNR 2026-04-10 + 17 new V2-baseline rows added to matrix on vv5dev 2026-04-22 + 4 custom-query rows baselined on vv5dev 2026-04-24 (WS-14 new) + 18 backlog — WS-11/12/13 new 2026-04-20; WS-13 unblocked 2026-04-22 after SP-2/SP-4 baseline landed). **V1/V2 parity confirmed (2026-04-22)**: vvdemo V1 (`b18dbfdb`) vs vv5dev V2 (`f36b65dd`) audit at build `f36b65dd` → **135/135 IDENTICAL / 0 unflagged divergences** (after classify() normalizes matrix `"undefined"`/`"null"` no-assertion markers to empty). No regression on the API write path. The `useUpdatedCalendarValueLogic` toggle is Forms-only. |
